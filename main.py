...
class Zefoy:
	def __init__(self):
		...
		self.url = 'None'
		self.text = 'By @KawaiiKillar [No url]'
	...
	def solve_captcha(self, path_to_file = None, b64 = None, delete_tag = ['\n','\r']):
		...
		if self.captcha_auto_solve:
			request = self.session.post("captcha.png", files={"file": ("captcha.png", open(task,'rb'), "image/png")}).json()
...
if os.path.exists('config.json') is False: open('config.json','w',encoding='utf-8',errors='ignore').write(json.dumps({'url':'https://www.tiktok.com/t/ZTRToxYc2','service':'Views','comment_id':None,'proxy':None,'captcha_auto_solve':False},indent=4))

webbrowser.open_new_tab('https://www.tiktok.com/@iron/video/7376686071742074121')
Z = Zefoy()
Z.check_config(True)
